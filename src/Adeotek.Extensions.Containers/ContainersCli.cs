...
public class ContainersCli
{
    ...
    public int CreateNetwork(NetworkConfig network, bool dryRun = false)
    {
        _containersCli.ClearArgsAndReset()
            ...
            .AddArgIf("--attachable", network.Attachable && _containersCli.IsDocker)
            .AddArgIf("--internal", network.Internal)
            .AddArgIf($"--ipam-driver {network.Ipam?.Driver}", !string.IsNullOrEmpty(network.Ipam?.Driver) && _containersCli.IsDocker)
