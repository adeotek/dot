...
public class ContainersManagerTests
{
    ...
    public void PurgeVolumes_WithSharedVolumes_DoNotRemoveAny()
    {
        ...
        var sut = GetContainersManager(out var shellProcessMock);
    ...
    private static ContainersManager GetContainersManager(out IShellProcess shellProcessMock)
    {
        var provider = TestHelpers.GetShellProcessProvider(out shellProcessMock);
        return new ContainersManager(CliFlavor, new ContainersCliCommand(provider));
    }
