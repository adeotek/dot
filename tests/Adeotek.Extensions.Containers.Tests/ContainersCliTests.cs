...
{
    private const string CliFlavor = "docker";
    ...
    public void ContainerExists_WithExisting_ReturnsTrue()
    {
        ...
        sut.OnContainersCliEvent += (_, e) =>
        {
            if (e is { Type: ContainersCliEventType.Command, Data.Count: 2 })
            {
    ...
    public void ContainerExists_WithUnknownError_ThrowsException()
    {
        ...
        Assert.Throws<ContainersCliException>(action);
        Assert.Equal(CliFlavor, cmd);
        Assert.Equal($"container inspect --format \"{{{{lower .Name}}}}\" {containerName}", args);
    }
    ...
    {
        var config = ContainersConfigManager.GetSampleConfig();
    ...
    public void AttachContainerToNetwork_WithAllArgs_ReturnOne()
    {
        ...
        sut.OnContainersCliEvent += (_, e) =>
        {
            if (e is { Type: ContainersCliEventType.ExitCode, Data.Count: 3 })
            {
                cmd = e.Data.GetValueOrDefault("cmd");
                args = e.Data.GetValueOrDefault("args");
            }
    ...
    public void StartContainer_WithMissing_ReturnsZero()
    {
        ...
        sut.OnContainersCliEvent += (_, e) =>
        {
            ...
            else if (e is { Type: ContainersCliEventType.Message, Data.Count: 2 })
            {
    ...
    {
        var network = ContainersConfigManager.GetSampleConfig().Networks.First().Value
    ...
    private static ContainersCli GetDockerCli(out IShellProcess shellProcessMock)
    {
        var provider = TestHelpers.GetShellProcessProvider(out shellProcessMock);
        return new ContainersCli(CliFlavor, new ContainersCliCommand(provider));
    }
