...
public class ContainersCliCommandTests
{
    private readonly ContainersCliCommand _sut;
    
    public ContainersCliCommandTests()
    {
        var provider = TestHelpers.GetShellProcessProvider(out _);
        _sut = new ContainersCliCommand(provider) { Command = "docker" };
    }
